...
class ReplayBuffer(object):
    ...
    def sample_commands(self, n, dr, dh):
        return [sample_command() for _ in range(n)]
...
def do_exploration(env, model, rb, writer, steps, n_episodes_per_iter, epsilon, max_return):
    ...
    exploration_cmds = rb.sample_commands(n_episodes_per_iter)
    ...
    for cmd in exploration_cmds:
        roll = rollout(1, env=env, model=model, cmd=cmd,
        ...
        steps += roll.length
        rewards += roll.mean_reward
        lengths += roll.mean_length
