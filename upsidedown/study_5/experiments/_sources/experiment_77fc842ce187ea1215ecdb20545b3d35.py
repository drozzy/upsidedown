...
class Behavior(nn.Module):
    ...
    def forward(self, prev_action, state, dr, dh):
        output_prev_action = torch.tanh(self.emb_prev_action(prev_action))
        # attn_scores = self.fc_attn_scores(state)
        output_state = torch.tanh(self.fc_state(state))
        output_dr = torch.tanh(self.fc_dr(dr * self.return_scale))
        output_dh = torch.tanh(self.fc_dh(dh * self.horizon_scale))

        # output = torch.cat([output_dr, output_dh], 1)
        output = output_prev_action * output_state * (output_dr * 0.9 + output_dh * 0.1)
