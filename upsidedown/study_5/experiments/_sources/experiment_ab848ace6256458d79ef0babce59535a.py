...
class Behavior(nn.Module):
    ...
    def forward(self, prev_action, state, dr, dh):
        output_prev_action = swish(self.emb_prev_action(prev_action))
        # attn_scores = self.fc_attn_scores(state)
        output_state = swish(self.fc_state(state))
        output_dr = swish(self.fc_dr(dr * self.return_scale))
        output_dh = swish(self.fc_dh(dh * self.horizon_scale))
