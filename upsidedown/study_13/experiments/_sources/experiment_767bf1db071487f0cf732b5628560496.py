...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size, device):
        trajectories = np.random.choice(self.buffer[:self.last_few], batch_size, replace=True)
        prev_action_batch = []
        action_batch = []
        s_batch  = []
        dr_batch = []
...
def do_exploration(env, model, rb, writer, steps, n_episodes_per_iter, epsilon, max_return):
    ...
    # Change our last few based on what the std is:
    ...
    writer.add_scalar('Buffer/last_few', rb.last_few, steps)    
    writer.add_scalar('Buffer/reward_mean', mean, steps)    
    writer.add_scalar('Buffer/reward_std', std, steps)    
    writer.add_scalar('Buffer/reward_mean_last_few', mean_last, steps)    
    writer.add_scalar('Buffer/reward_std_last_few', std_last, steps)    
